    ...
    for i in $(seq $start $end); do
        if [ $i -lt ${#experiments[@]} ]; then
            ...
            python main.py \
                ...
                --method "$method" \
                --renyi_loss "$renyi" \
                --log_file "./log/citeseer/log_${method}_${renyi}.txt" &
        fi
    done
}
...
for ((i=0; i<total; i+=10)); do
    echo "Lanzando grupo de experimentos $((i/10 + 1)), falta $(($total - i))"
    launch_group $i $((i+9))
    sleep 5  # Esperar 5 segundos antes del siguiente grupo
